/* ===========================================================
   stc — منطق التحقق من استحقاق بوكسيات الألياف البصرية (ODB)
   للعرض الخاص بسعر 172 ريال
...
(function () {
  ...
  // قاعدة البيانات (تأتي من data.js)
  var rawCodes = (typeof BOX_CODES !== "undefined") ? BOX_CODES : [];

  // مصفوفة موازية بالأكواد بعد التطبيع — تُبنى مرة واحدة فقط لتسريع البحث
  var normCodes = new Array(rawCodes.length);
  for (var i = 0; i < rawCodes.length; i++) {
    normCodes[i] = normalize(rawCodes[i]);
  }
  var codeSet = new Set(normCodes);
  ...
  var suggestionsHd = document.getElementById("suggestionsHeading");
  var suggestionsLs = document.getElementById("suggestionsList");
  ...
  function hideSuggestions() {
    ...
    suggestionsLs.innerHTML = "";
  }

  // البحث عن أقرب البوكسيات بنفس الترتيب/التنسيق:
  // نبدأ بمطابقة الأحرف كاملة من بداية الرقم (startsWith)، وإذا لم نجد
  // أي تطابق نقصّر الرقم المُدخل حرفاً حرفاً من نهايته حتى نصل لمجموعة نتائج.
  function findClosestMatches(normalized) {
    var query = normalized;
    var results = [];

    while (query.length > 0 && results.length === 0) {
      for (var i = 0; i < normCodes.length; i++) {
        if (normCodes[i].indexOf(query) === 0) {
          results.push(rawCodes[i]);
    ...
    return { matches: results, matchedPrefix: query };
  }

  function renderSuggestions(normalized) {
    var found = findClosestMatches(normalized);
    ...
    suggestionsHd.textContent = "أقرب البوكسيات بنفس التنسيق (" + found.matchedPrefix + "):";
    suggestionsLs.innerHTML = "";

    found.matches.forEach(function (code) {
      ...
      suggestionsLs.appendChild(item);
    });
  ...
  function checkBox() {
    ...
    runFiberScan(function () {
      ...
      if (codeSet.has(normalized)) {
        showResult("ok", "🎉 هذا البوكس مستحق للعرض الخاص بسعر 172 ريال", raw);
      } else {
        showResult("no", "عذراً، هذا البوكس غير مستحق للعرض الخاص حالياً", raw);
